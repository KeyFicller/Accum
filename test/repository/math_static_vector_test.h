...
TEST(Vector3DTest, SwizzleXY)
{
    ...
    EXPECT_EQ(xy[0], 10);
    EXPECT_EQ(xy[1], 20);
    static_assert(std::is_same<decltype(xy), vec_impl_t<int, 2>>::value, "Should return vec_impl_t<int, 2>");
}
...
TEST(Vector4DTest, SwizzleXYZ)
{
    ...
    static_assert(std::is_same<decltype(xyz), vec_impl_t<double, 3>>::value, "Should return vec_impl_t<double, 3>");
}
