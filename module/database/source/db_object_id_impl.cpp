...
namespace PRJ_NAME
{
    ...
    db_object_id_impl::db_object_id_impl(db_object_id &_facade, db_handle _handle)
        : implement_t<db_object_id>(_facade), m_handle(_handle)
    {
    }
    ...
    bool db_object_id_impl::is_valid() const
    {
        int ret = true;
    ...
    db_object_id_impl::operator bool() const
    {
        return is_valid();
    }

    bool db_object_id_impl::operator==(const db_object_id &_other) const
    {
