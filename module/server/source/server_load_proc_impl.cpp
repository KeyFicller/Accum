...
namespace PRJ_NAME
{
    ...
    bool load_proc_impl::end()
    {
        ASSERT(m_pipeline.size() == 1);
        m_pipeline.pop();
    }

    void load_proc_impl::begin_sequence()
    {
    ...
    void load_proc_impl::end_sequence()
    {
        ASSERT(!has_value());
        m_pipeline.pop();
    }

    bool load_proc_impl::has_value() const
    {
        return !m_pipeline.top().is_anchor;
    }

    bool load_proc_impl::has_key(const std::string &_value) const
    {
        return has_value() && !m_pipeline.top().node[_value].IsNull();
    }

    void load_proc_impl::out_key()
    {
        wait_for_key() = true;
    }

    void load_proc_impl::out(const std::string &_value)
    {
        ASSERT(wait_for_key());
        wait_for_key() = false;
    ...
    void load_proc_impl::out(std::string& _value)
    {
        _value = m_pipeline.top().node.as<std::string>();
        m_pipeline.pop();
    }

    void load_proc_impl::out(int &_value)
    {
        _value = m_pipeline.top().node.as<int>();
        m_pipeline.pop();
    }

    void load_proc_impl::out(float &_value)
    {
        _value = m_pipeline.top().node.as<float>();
        m_pipeline.pop();
    }

    void load_proc_impl::out(bool &_value)
    {
